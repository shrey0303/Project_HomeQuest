...
const CreateListingPage = () => {
  ...
  const [createDetails] = useMutation(CREATE_DETAILS, {
    ...
    onError: (error) => {
      console.log(error);
      toast.error("Somethings went wrong in detail.");
    }
  });
  ...
  const [createLocation] = useMutation(CREATE_LOCATION, {
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        if (res?.message) return toast.error("Somethings went wrong in image formation.");
    ...
    onError: (error) => {
      ...
      toast.error("Somethings went wrong in location.");
    }
  });
